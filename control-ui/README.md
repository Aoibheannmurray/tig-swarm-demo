...
Svelte + Vite, no framework beyond Svelte. Two build entries:
...
- `admin/index.html` → **Admin Console** (mode `hosted`). invite / revoke /
  list contributors, switch challenge, broadcast, seed pools. Served by the
  swarm's own hosted FastAPI server at `/admin/`; talks to the existing
  `/api/*` + `/api/admin/*` (admin key kept in `sessionStorage`).
...
- Hosted Admin Console: the root `Dockerfile` builds this and copies
  `dist/admin` + `dist/assets` into the server image's `static/`, so the swarm
  server serves the console at `/admin/`. (The companion's `dist/index.html` is
  deliberately NOT copied — it would shadow the dashboard's own `index.html`.)
