...
async def test_improving_but_negative_line_is_culled():
    ...
    state = await server.get_state(agent_id="agentA", challenge=CHALLENGE)
    ...
    state2 = await server.get_state(agent_id="agentA", challenge=CHALLENGE)
...
async def test_positive_line_is_not_culled():
    ...
    for i, score in enumerate((-100.0, 50.0, 60.0)):
        await server.create_iteration(_iter(server, "agentA", score, f"edit {i}"))
...
async def test_stagnation_reset_reports_stagnation_reason():
    ...
    await server.create_iteration(_iter(server, "agentA", 100.0, "baseline"))
    # Two non-improving edits -> runs_since_improvement = 2 = stagnation_limit.
    await server.create_iteration(_iter(server, "agentA", 90.0, "worse 1"))
    await server.create_iteration(_iter(server, "agentA", 80.0, "worse 2"))
