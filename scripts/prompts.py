...
start with the same `use` imports and compile.{rust_rules}"""
...
def parse_hyperparameter_response(response: str) -> dict:
    ...
    if "use tig_challenges::" not in variant_code:
        return {**fail, "error": "variant mod.rs missing the `use tig_challenges::...` import"}
    return {
        "ok": True,
        "error": "",
